...
export const applyToShelterInDB = async (
  ...
  if (existingApp) {
    ...
    // Re-apply flow after rejection/leave.
...
export const leaveShelterInDB = async (
  userId: string,
  shelterId: string,
  ip?: string,
  userAgent?: string,
  ...
  const hasMembership =
    user.memberships?.some((m) => m.shelterId.toString() === shelterId) || false;
  ...
  user.memberships = (user.memberships || []).filter(
    (m) => m.shelterId.toString() !== shelterId,
  );

  if (user.shelterId?.toString() === shelterId) {
    const fallbackMembership = user.memberships.find(
      (m) => m.role === "shelter_staff",
    );
    user.shelterId = fallbackMembership?.shelterId;
  }

  // Keep shelter_staff role so they can apply/join another shelter.
