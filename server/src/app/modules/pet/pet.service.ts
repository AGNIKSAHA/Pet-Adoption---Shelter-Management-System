...
export const createPetInDB = async (data: Omit<Partial<IPet>, "shelterId"> & {
    shelterId?: string | mongoose.Types.ObjectId;
}, userId: string, userRole: string, userShelterId?: string, ip?: string, userAgent?: string) => {
    let { shelterId } = data;
    if (userRole === "shelter_staff") {
        ...
        if (!application) {
            throw new AppError(403, "You must be an approved staff member for this shelter to add pets");
        }
    }
    if (!shelterId) {
...
export const getPetsFromDB = async (filters: Record<string, unknown>) => {
    const { species, breed, minAge, maxAge, gender, size, status, shelterId, goodWithKids, goodWithDogs, goodWithCats, page = 1, limit = 20, } = filters;
    const query: FilterQuery<IPet> & {
        isActive?: boolean;
    } = { isActive: true };
    if (species)
    ...
    if (minAge || maxAge) {
        query.age = {};
        if (minAge)
            query.age.$gte = parseInt(minAge as string);
        if (maxAge)
            query.age.$lte = parseInt(maxAge as string);
    }
    if (goodWithKids === "true")
        query["compatibility.goodWithKids"] = true;
    if (goodWithDogs === "true")
        query["compatibility.goodWithDogs"] = true;
    if (goodWithCats === "true")
        query["compatibility.goodWithCats"] = true;
...
export const getPetStatsFromDB = async (userRole: string, userShelterId?: string, queryShelterId?: string) => {
    const shelterId = userRole === "admin" ? queryShelterId : queryShelterId || userShelterId;
    const query: FilterQuery<IPet> & {
        isActive?: boolean;
    } = { isActive: true };
    if (shelterId)
        query.shelterId = new mongoose.Types.ObjectId(shelterId);
...
export const getPetByIdFromDB = async (id: string) => {
    const pet = await Pet.findById(id).populate("shelterId", "name email phone address location");
    if (!pet) {
        throw new AppError(404, "Pet not found");
    }
    const shelterId = typeof pet.shelterId === "object" && "_id" in pet.shelterId
        ? (pet.shelterId as {
            _id: unknown;
        })._id
        : pet.shelterId;
    const contactPerson = await User.findOne({
        ...
        $or: [
            { role: "admin" },
            { role: "shelter_staff", shelterApprovalStatus: "approved" },
        ],
    })
        .sort({ role: 1 })
...
export const updatePetInDB = async (id: string, updates: Partial<IPet>, userId: string, userRole: string, _userShelterId?: string, ip?: string, userAgent?: string) => {
    ...
    if (userRole === "shelter_staff") {
        ...
        if (!application) {
            throw new AppError(403, "You must be an approved staff member for this shelter to update pets");
        }
    }
...
export const updatePetStatusInDB = async (id: string, status: string, userId: string, userRole: string, _userShelterId?: string, ip?: string, userAgent?: string) => {
    const pet = await Pet.findById(id);
    if (!pet) {
        throw new AppError(404, "Pet not found");
    }
    if (userRole === "shelter_staff") {
        ...
        if (!application) {
            throw new AppError(403, "You must be an approved staff member for this shelter to update pet status");
        }
    }
    if (!(pet as IPet).canTransitionTo(status as PetStatus)) {
        throw new AppError(400, `Invalid status transition from ${pet.status} to ${status}`);
    }
    if (pet.status === "medical_hold" && status === "available") {
        throw new AppError(400, "Vet sign-off required. Submit vet approval request to release from medical hold.");
    }
...
export const requestVetMedicalReleaseInDB = async (petId: string, vetEmail: string, requestNote: string | undefined, userId: string, userRole: string, ip?: string, userAgent?: string) => {
    const pet = await Pet.findById(petId).populate("shelterId", "name");
    if (!pet) {
        throw new AppError(404, "Pet not found");
    }
    if (pet.status !== "medical_hold") {
        throw new AppError(400, "Vet sign-off is only required when pet is in medical hold");
    }
    if (userRole === "shelter_staff") {
        const { StaffApplication } = await import("../shelter/staff-application.model");
        const application = await StaffApplication.findOne({
        ...
        if (!application) {
            throw new AppError(403, "You must be an approved staff member for this shelter to request vet sign-off");
        }
    }
    ...
    if (existingPending) {
        throw new AppError(400, "A pending vet sign-off request already exists for this pet");
    }
    ...
    const serverBaseUrl = process.env.SERVER_PUBLIC_URL || `http://localhost:${env.PORT}`;
    ...
    const shelterName = typeof pet.shelterId === "object" && "name" in pet.shelterId
        ? (pet.shelterId as {
            name?: string;
        }).name || "Shelter"
        : "Shelter";
...
export const processVetMedicalReleaseInDB = async (token: string, decision: "approve" | "reject") => {
    const nextStatus = decision === "approve" ? "approved" : "rejected";
    const approval = await PetVetApproval.findOneAndUpdate({ token, status: "pending" }, { $set: { status: nextStatus, decidedAt: new Date() } }, { new: true });
    if (!approval) {
...
const getApprovedStaffShelterIds = async (userId: string) => {
    const { StaffApplication } = await import("../shelter/staff-application.model");
    const apps = await StaffApplication.find({ userId, status: "approved" }).select("shelterId");
    return apps.map((app) => app.shelterId.toString());
};
export const createPetTransferRequestInDB = async (petId: string, toShelterId: string, note: string | undefined, userId: string, ip?: string, userAgent?: string) => {
...
export const respondPetTransferRequestInDB = async (requestId: string, decision: "approved" | "rejected", decisionNote: string | undefined, userId: string, ip?: string, userAgent?: string) => {
    ...
    try {
        ...
        await pet.save();
        await Application.updateMany({ petId: request.petId }, { $set: { shelterId: request.toShelterId } });
        applicationsMoved = true;
        await FosterAssignment.updateMany({ petId: request.petId }, { $set: { shelterId: request.toShelterId } });
        ...
        if (applicationsMoved) {
            await Application.updateMany({ petId: request.petId }, { $set: { shelterId: request.fromShelterId } }).catch(() => null);
        }
        if (assignmentsMoved) {
            await FosterAssignment.updateMany({ petId: request.petId }, { $set: { shelterId: request.fromShelterId } }).catch(() => null);
        }
...
export const deletePetInDB = async (id: string, userId: string, userRole: string, _userShelterId?: string, ip?: string, userAgent?: string) => {
    const pet = await Pet.findById(id);
    if (!pet) {
        throw new AppError(404, "Pet not found");
    }
    if (userRole === "shelter_staff") {
        ...
        if (!application) {
            throw new AppError(403, "You must be an approved staff member for this shelter to delete pets");
        }
    }
    pet.isActive = false;
    await pet.save();
...
export const searchNearbyInDB = async (longitude: string, latitude: string, maxDistance: string, page: string, limit: string) => {
