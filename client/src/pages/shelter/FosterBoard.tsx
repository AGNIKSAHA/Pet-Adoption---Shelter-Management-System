...
export default function FosterBoard() {
    ...
    const currentShelterName = allApproved.find((s) => s.id === normalizedActiveShelterId)?.name ||
        "Shelter";
    const [activeTab, setActiveTab] = useState<"assignments" | "parents">("assignments");
    const [isAssignModalOpen, setIsAssignModalOpen] = useState(false);
    const [selectedAssignmentToReturn, setSelectedAssignmentToReturn] = useState<FosterAssignment | null>(null);
    ...
    const completeAssignmentMutation = useMutation({
        mutationFn: ({ id, returnStatus, notes, }: {
            id: string;
            returnStatus: string;
            notes?: string;
    ...
    return (<div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex gap-2">
          <button onClick={() => setIsAssignModalOpen(true)} className="btn btn-primary flex items-center gap-2">
      ...
      <div className="flex gap-4 border-b border-gray-200">
        <button onClick={() => setActiveTab("assignments")} className={`pb-4 px-2 text-sm font-medium transition-colors relative ${activeTab === "assignments"
            ? "text-primary-600 border-b-2 border-primary-600"
            : "text-gray-500 hover:text-gray-700"}`}>
          Active Assignments
        </button>
        <button onClick={() => setActiveTab("parents")} className={`pb-4 px-2 text-sm font-medium transition-colors relative ${activeTab === "parents"
            ? "text-primary-600 border-b-2 border-primary-600"
            : "text-gray-500 hover:text-gray-700"}`}>
          Foster Parents
        </button>
      ...
      {activeTab === "assignments" ? (<AssignmentsGrid isLoading={assignmentsLoading} assignments={assignments} onTakeBack={setSelectedAssignmentToReturn} isTakeBackPending={completeAssignmentMutation.isPending}/>) : (<FosterTable isLoading={fostersLoading} fosters={fosters} onUpdateStatus={(id, status) => updateStatusMutation.mutate({ id, status })} onToggleActive={(id, isActive) => toggleActiveMutation.mutate({ id, isActive })} onDelete={(id) => deleteFosterMutation.mutate(id)}/>)}

      <AssignPetModal isOpen={isAssignModalOpen} onClose={() => setIsAssignModalOpen(false)} onSubmit={(data) => assignMutation.mutate(data)} isPending={assignMutation.isPending} fosters={fosters} availablePets={availablePets?.data?.pets || []}/>

      <TakeBackModal assignment={selectedAssignmentToReturn} onClose={() => setSelectedAssignmentToReturn(null)} onSubmit={(data) => completeAssignmentMutation.mutate({
            id: selectedAssignmentToReturn?._id || "",
            ...data,
        })} isPending={completeAssignmentMutation.isPending} returnStatus={returnStatus} setReturnStatus={setReturnStatus} returnNotes={returnNotes} setReturnNotes={setReturnNotes}/>
    </div>);
}
