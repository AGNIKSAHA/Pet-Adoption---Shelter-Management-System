...
export const getAllUsersFromDB = async (page: number, limit: number, role?: string, search?: string) => {
...
export const updateUserStatusInDB = async (id: string, data: {
    role?: string;
...
export const createShelterInDB = async (data: Partial<IShelter>, adminId: string) => {
    return await Shelter.create({ ...data, createdBy: adminId });
};
export const updateShelterInDB = async (id: string, data: Partial<IShelter>, adminId: string) => {
    const existingShelter = await Shelter.findById(id);
    if (!existingShelter) {
        throw new AppError(404, "Shelter not found");
    }
    if (existingShelter.createdBy &&
...
export const deleteShelterFromDB = async (id: string, adminId: string) => {
    ...
    for (const user of affectedUsers) {
        ...
        const hadShelterMembership = currentMemberships.some((m) => m.shelterId.toString() === id);
        user.memberships = currentMemberships.filter((m) => m.shelterId.toString() !== id);
        if (user.shelterId && user.shelterId.toString() === id) {
            const nextStaffMembership = user.memberships.find((m) => m.role === "shelter_staff");
...
export const approveShelterRequestInDB = async (applicationId: string, adminId: string) => {
    const application = await StaffApplication.findById(applicationId).populate("shelterId", "createdBy");
    if (!application) {
    ...
    if (!shelter?.createdBy || shelter.createdBy.toString() !== adminId) {
        throw new AppError(403, "You can only approve requests for shelters that you created");
    }
    ...
    if (user) {
        const exists = user.memberships?.find((m) => m.shelterId.toString() === application.shelterId.toString());
        if (!exists) {
            if (!user.memberships)
                user.memberships = [];
...
export const rejectShelterRequestInDB = async (applicationId: string, adminId: string) => {
    const application = await StaffApplication.findById(applicationId).populate("shelterId", "createdBy");
    if (!application) {
        throw new AppError(404, "Application not found");
    }
    ...
    if (!shelter?.createdBy || shelter.createdBy.toString() !== adminId) {
        throw new AppError(403, "You can only reject requests for shelters that you created");
    }
