...
export const triggerWebhooks = async (event: string, data: Record<string, unknown>, userId?: string) => {
    try {
        ...
        const promises = webhooks.map((webhook) => sendWebhook(webhook, payload).catch((error) => {
...
export const createWebhook = async (userId: string, url: string, events: string[]) => {
...
export const updateWebhook = async (webhookId: string, userId: string, updates: {
    url?: string;
    events?: string[];
    isActive?: boolean;
}) => {
