...
export const buildApplicationStatusDedupKey = (applicationId: string, status: string) => `application-status:${applicationId}:${status.toLowerCase()}`;
export const enqueueApplicationStatusEmail = async (params: {
    ...
    const outbox = await EmailOutbox.findOneAndUpdate({ dedupKey }, {
        $setOnInsert: {
            dedupKey,
    ...
    await emailQueue.add({ outboxId: outbox._id.toString() }, {
...
export const processOutboxEmailById = async (outboxId: string) => {
    ...
    const claim = await EmailOutbox.findOneAndUpdate({ _id: outboxId, status: { $in: ["pending", "failed"] } }, { $set: { status: "sending", sendingAt: new Date() } }, { new: true });
    if (!claim) {
    ...
    try {
        ...
        await EmailOutbox.updateOne({ _id: claim._id, status: "sending" }, {
            $set: {
                status: "sent",
                sentAt: new Date(),
                lastError: undefined,
        ...
        await EmailOutbox.updateOne({ _id: claim._id }, {
            $set: {
                status: "failed",
                lastError: message,
            },
