...
import { Plus, MapPin, Mail, Phone, Edit2, Trash2, Building2, Users, Search, Loader2, } from "lucide-react";
...
export default function ShelterManagement() {
    ...
    const filteredShelters = shelters?.filter((s: Shelter) => s.name.toLowerCase().includes(search.toLowerCase()) ||
    ...
    return (<div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <button onClick={handleAdd} className="btn btn-primary flex items-center gap-2 self-start md:self-auto shadow-lg shadow-primary-100">
      ...
      <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm relative">
        ...
        <input type="text" placeholder="Search shelters by name or location..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full pl-12 pr-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-primary-500 outline-none"/>
      </div>

      
      {isLoading ? (<div className="flex justify-center py-12">
          <Loader2 className="w-10 h-10 text-primary-600 animate-spin"/>
        </div>) : filteredShelters.length === 0 ? (<div className="text-center py-20 bg-white rounded-2xl border-2 border-dashed border-gray-100">
          <Building2 className="w-16 h-16 text-gray-200 mx-auto mb-4"/>
          <p className="text-gray-500 font-medium">
            No shelters found. Try a different search or add one.
          </p>
        </div>) : (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredShelters.map((shelter: Shelter) => (<div key={shelter._id} className="bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-all group">
              <div className="p-6 space-y-4">
                <div className="flex justify-between items-start">
                  ...
                  <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => handleEdit(shelter)} className="p-2 hover:bg-gray-100 rounded-lg text-gray-400 hover:text-primary-600 transition-all" title="Edit Shelter">
                ...
                }} className="p-2 hover:bg-red-50 rounded-lg text-gray-400 hover:text-red-600 transition-all" title="Delete Shelter">
                ...
                <div className="pt-4 border-t border-gray-50 flex items-center justify-between text-sm">
                  ...
                  <div className={`w-2.5 h-2.5 rounded-full ${shelter.isActive ? "bg-green-500" : "bg-red-500"} ring-4 ring-gray-50`}/>
                </div>
              </div>
            </div>))}
        </div>)}

      <ShelterModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onSubmit={handleSubmit} shelter={selectedShelter} isLoading={createMutation.isPending || updateMutation.isPending}/>
    </div>);
}
