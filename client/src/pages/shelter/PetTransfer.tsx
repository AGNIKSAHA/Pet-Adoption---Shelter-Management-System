...
export default function PetTransfer() {
    ...
    const shelterOptions = useMemo(() => allShelters.filter((shelter) => shelter._id !== currentShelterId), [allShelters, currentShelterId]);
    ...
    return (<div className="space-y-6">
      ...
      <div className="bg-white border border-gray-100 rounded-xl p-4 space-y-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          <select value={petId} onChange={(e) => setPetId(e.target.value)} className="px-3 py-2 border border-gray-200 rounded-lg">
            <option value="">Select pet</option>
            {pets.map((pet) => (<option key={pet._id} value={pet._id}>
                {pet.name} ({pet.species})
              </option>))}
          </select>

          <select value={toShelterId} onChange={(e) => setToShelterId(e.target.value)} className="px-3 py-2 border border-gray-200 rounded-lg">
            <option value="">Select target shelter</option>
            {shelterOptions.map((shelter) => (<option key={shelter._id} value={shelter._id}>
                {shelter.name}
              </option>))}
          </select>

          <input value={note} onChange={(e) => setNote(e.target.value)} className="px-3 py-2 border border-gray-200 rounded-lg" placeholder="Optional note"/>
        </div>

        <button onClick={() => createTransferMutation.mutate()} disabled={!petId || !toShelterId || createTransferMutation.isPending} className="btn btn-primary disabled:opacity-60">
          Send Request
        </button>
      </div>
      ...
      <div className="bg-white border border-gray-100 rounded-xl p-4 space-y-3">
        ...
        <input value={decisionNote} onChange={(e) => setDecisionNote(e.target.value)} className="px-3 py-2 border border-gray-200 rounded-lg w-full md:w-1/2" placeholder="Decision note (optional)"/>
        <div className="space-y-2">
          {incoming.length === 0 ? (<p className="text-sm text-gray-500">No incoming requests.</p>) : (incoming.map((request) => {
            const pet = request.petId as Pet;
            const fromShelter = request.fromShelterId as Shelter;
            return (<div key={request._id} className="p-3 border border-gray-200 rounded-lg flex flex-col md:flex-row md:items-center md:justify-between gap-3">
      ...
      <div className="bg-white border border-gray-100 rounded-xl p-4 space-y-3">
        ...
        <div className="space-y-2">
          {outgoing.length === 0 ? (<p className="text-sm text-gray-500">No outgoing requests.</p>) : (outgoing.map((request) => {
            const pet = request.petId as Pet;
            const toShelter = request.toShelterId as Shelter;
            return (<div key={request._id} className="p-3 border border-gray-200 rounded-lg">
