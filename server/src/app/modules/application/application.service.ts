...
import { buildApplicationStatusDedupKey, enqueueApplicationStatusEmail, } from "../../common/utils/email-outbox.util";
export const createApplicationInDB = async (data: {
    petId: string;
    ...
    if (existingApp) {
        throw new AppError(409, "You already have an active application for this pet");
    }
...
export const getApplicationsFromDB = async (userId: string, role: string, shelterIdFromToken?: string, filters: Record<string, unknown> = {}) => {
...
export const getApplicationByIdFromDB = async (id: string, userId: string, role: string, shelterIdFromToken?: string) => {
...
export const updateApplicationStatusInDB = async (id: string, data: {
    status: ApplicationStatus;
    notes?: string;
    expectedVersion?: number;
}, userId: string, role: string, shelterIdFromToken?: string, ip?: string, userAgent?: string) => {
    const { status, notes, expectedVersion } = data;
    const application = await Application.findById(id).populate("petId adopterId");
    if (!application) {
        throw new AppError(404, "Application not found");
    }
    if (role === "shelter_staff") {
    ...
    if (!Number.isInteger(expectedVersion)) {
        throw new AppError(400, "expectedVersion is required for concurrent-safe reviews");
    }
    ...
    const updatedWithLock = await Application.findOneAndUpdate({ _id: id, __v: expectedVersion }, {
        $set: updatePayload,
        $inc: { __v: 1 },
    }, { new: true });
    if (!updatedWithLock) {
        const latest = await Application.findById(id)
            ...
            .populate("shelterId", "name")
            .populate("reviewedBy", "firstName lastName");
        throw new AppError(409, "Merge conflict: this application was updated by another reviewer", true, {
            code: "APPLICATION_VERSION_CONFLICT",
            currentApplication: latest,
        });
    }
    const updatedApplication = await Application.findById(id).populate("petId adopterId");
    ...
    enqueueApplicationStatusEmail({
        dedupKey: buildApplicationStatusDedupKey(updatedApplication._id.toString(), status),
...
export const withdrawApplicationInDB = async (id: string, userId: string, ip?: string, userAgent?: string) => {
    const application = await Application.findById(id);
    if (!application) {
        throw new AppError(404, "Application not found");
    }
