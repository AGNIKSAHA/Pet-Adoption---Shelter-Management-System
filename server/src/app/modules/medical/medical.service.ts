...
const ensureStaffAccessToPet = async (userId: string, petId: string) => {
  ...
  if (!application) {
    throw new AppError(
      403,
      "You must be approved staff for this shelter to manage medical records",
    );
  }
...
export const createMedicalRecordInDB = async (
  petId: string,
  ...
  if (data.recordType === "correction") {
    throw new AppError(
      400,
      "Use the correction endpoint to create vaccination corrections",
    );
  }
  ...
  if (data.recordType === "vaccination" && !hasPdfDocument(data.documents)) {
    throw new AppError(
      400,
      "Vaccination report PDF is required to submit vaccination records",
    );
  }
...
export const createVaccinationCorrectionInDB = async (
  petId: string,
  originalRecordId: string,
  data: Omit<CreateMedicalRecordInput, "recordType">,
  userId: string,
  userRole: string,
  ip?: string,
  userAgent?: string,
  ...
  if (!hasPdfDocument(data.documents)) {
    throw new AppError(
      400,
      "Vaccination correction requires a PDF report copy",
    );
  }
...
export const updateMedicalRecordInDB = async (
  id: string,
  ...
  if (record.recordType === "vaccination") {
    throw new AppError(
      400,
      "Vaccination records are immutable. Add a correction record instead.",
    );
  }
...
export const deleteMedicalRecordInDB = async (
  id: string,
  userId: string,
  userRole: string,
  ip?: string,
  ...
  if (record.recordType === "vaccination" || record.recordType === "correction") {
    throw new AppError(
      400,
      "Vaccination and correction records are immutable and cannot be deleted",
    );
  }
...
export const getMedicalTimelineFromDB = async (
  petId: string,
  userId: string,
  userRole: string,
): Promise<{
  ...
  type MedicalRecordLean = {
    ...
    documents: string[];
    nextDueDate?: Date;
    correctsRecordId?: { toString: () => string };
