...
const uploadDataImageAndGetUrl = async (api: AxiosInstance, dataUrl: string, index: number) => {
...
const normalizePhotoArrayForSync = async (api: AxiosInstance, photos: unknown) => {
...
export const queueOfflinePetOperation = (operation: Omit<OfflinePetOperation, "id" | "createdAt">) => {
    ...
    for (const op of queue) {
        try {
            if (op.type === "create") {
                ...
                createPayload.photos = await normalizePhotoArrayForSync(api, createPayload.photos);
                await api.post("/pets", {
                    ...createPayload,
                ...
                petDataWithoutStatus.photos = await normalizePhotoArrayForSync(api, petDataWithoutStatus.photos);
