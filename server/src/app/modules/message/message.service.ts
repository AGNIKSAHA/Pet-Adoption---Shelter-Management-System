...
} from "./message-conversation.model";
...
interface RequestUser {
  ...
  roles?: string[];
  memberships?: { shelterId: string; role: "admin" | "shelter_staff" | "adopter" }[];
}
...
const hasShelterAccess = async (userId: string, shelterId: string) => {
  ...
  const inMembership = (user.memberships || []).some(
    (m) => m.shelterId.toString() === shelterId && ["admin", "shelter_staff"].includes(m.role),
  );
...
const resolveShelterForStaff = async (
  userId: string,
  ...
  const membershipShelter = (user.memberships || []).find((m) =>
    ["admin", "shelter_staff"].includes(m.role),
  );
...
const resolveShelterForStaffUser = async (staffUserId: string) => {
  ...
  if (!user) throw new AppError(404, "Staff user not found");

  if (user.shelterId) return user.shelterId.toString();

  const m = (user.memberships || []).find((item) =>
    ["admin", "shelter_staff"].includes(item.role),
  );
  if (m?.shelterId) return m.shelterId.toString();
  ...
  if (app?.shelterId) return app.shelterId.toString();
...
const canAccessConversation = async (
  ...
  options?: { requireAssignedStaff?: boolean },
) => {
...
export const getConversationsFromDB = async (currentUser: RequestUser) => {
  ...
  const formatted = await Promise.all(
    conversations.map((conversation) => formatConversation(conversation, currentUser)),
  );

  return formatted;
};

export const startConversationInDB = async (
  currentUser: RequestUser,
  receiverId?: string,
  shelterId?: string,
) => {
  const canActAsAdopter =
    currentUser.role === "adopter" || (currentUser.roles || []).includes("adopter");
...
export const sendMessageToConversationInDB = async (
  currentUser: RequestUser,
  conversationId: string,
  content: string,
) => {
  ...
  if (currentUser.role === "adopter") {
    const shelterStaff = await getShelterStaffCandidates(
      conversation.shelterId._id.toString(),
    );
    if (shelterStaff.length === 0) {
...
export const markConversationAsReadInDB = async (
  currentUser: RequestUser,
  conversationId: string,
) => {
  const conversation = await getConversationById(conversationId);
...
export const handoffConversationInDB = async (
  currentUser: RequestUser,
  conversationId: string,
  toStaffId: string,
  reason?: string,
  ...
  const canTargetAccess = await hasShelterAccess(
    toStaffId,
    conversation.shelterId._id.toString(),
  );
  ...
  const updated = await MessageConversation.findByIdAndUpdate(
    conversationId,
    {
...
export const getConversationStaffOptionsFromDB = async (
  currentUser: RequestUser,
  conversationId: string,
) => {
  const conversation = await getConversationById(conversationId);
...
// Legacy direct message methods kept for existing non-shelter message flows.
...
export const markAsReadInDB = async (receiverId: string, senderId: string) => {
  return Message.updateMany(
    { senderId, receiverId, isRead: false, conversationId: { $exists: false } },
    { $set: { isRead: true } },
  );
};
