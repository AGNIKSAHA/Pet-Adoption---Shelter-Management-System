...
export default function Profile() {
    ...
    const applyMutation = useMutation({
        mutationFn: (shelterId: string) => api.post("/shelters/apply", { shelterId }),
        onSuccess: () => {
    ...
    const getApplicationStatus = (shelterId: string) => {
        return user.staffApplications?.find((app) => {
            if (!app.shelterId)
                return false;
            const id = typeof app.shelterId === "string" ? app.shelterId : app.shelterId._id;
            return id === shelterId;
        });
    };
    const isCurrentShelterMember = (shelterId: string) => {
        const inMemberships = (user.memberships || []).some((m) => {
            if (!m.shelterId)
                return false;
            const id = typeof m.shelterId === "string" ? m.shelterId : m.shelterId._id;
            return id === shelterId;
        });
    ...
    return (<div className="max-w-4xl mx-auto space-y-6">
      <ProfileHeader isEditing={isEditing} setIsEditing={setIsEditing} isLoading={isLoading} handleCancel={handleCancel} onSubmit={handleSubmit(onSubmit)}/>

      <PersonalInfoForm user={user} activeRole={effectiveRole} isEditing={isEditing} register={register} errors={errors} onSubmit={handleSubmit(onSubmit)}/>

      {effectiveRole === "shelter_staff" && (<ShelterApplicationsList shelters={shelters} getApplicationStatus={getApplicationStatus} applyMutation={applyMutation} leaveMutation={leaveMutation} isCurrentShelterMember={isCurrentShelterMember}/>)}
    </div>);
}
