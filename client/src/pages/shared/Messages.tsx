...
export default function Messages() {
  const { user: currentUser, activeShelterId, activeRole } = useAppSelector(
    (state) => state.auth,
  );
  ...
  const handoffMutation = useMutation({
    mutationFn: async (toStaffId: string) => {
      ...
      const response = await api.patch(
        `/messages/conversations/${selectedConversationId}/handoff`,
        { toStaffId, reason: "Shift handoff" },
      );
      return response.data.data;
    },
    onSuccess: () => {
  ...
  const { data: staffOptions } = useQuery({
    ...
    queryFn: async () => {
      if (!selectedConversationId) return [];
      const response = await api.get(
        `/messages/conversations/${selectedConversationId}/staff-options`,
      );
  ...
  return (
    <div className="h-[calc(100vh-120px)] flex bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
      <ConversationSidebar
        ...
        onSelectSearchUser={handleSearchUserSelect}
        enableSearch={allowUserSearch}
      />
      ...
      <ChatWindow
        ...
        onHandoff={(toStaffId) => handoffMutation.mutate(toStaffId)}
