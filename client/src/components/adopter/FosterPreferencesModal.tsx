...
export default function FosterPreferencesModal({ isOpen, onClose, prefSpecies, setPrefSpecies, prefCapacity, setPrefCapacity, onSubmit, isPending, }: FosterPreferencesModalProps) {
    if (!isOpen)
        return null;
    return (<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className="bg-white rounded-2xl w-full max-w-md overflow-hidden shadow-2xl animate-in fade-in zoom-in duration-200">
        ...
        <div className="p-6 space-y-6">
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {["dog", "cat", "bird", "rabbit", "other"].map((species) => (<button key={species} onClick={() => {
                setPrefSpecies((prev) => prev.includes(species)
                    ? prev.filter((s) => s !== species)
                    : [...prev, species]);
            }} className={`flex items-center justify-between px-4 py-2.5 rounded-xl border text-sm font-semibold capitalize transition-all ${prefSpecies.includes(species)
                ? "bg-primary-50 border-primary-500 text-primary-700 ring-1 ring-primary-500"
                : "bg-white border-gray-200 text-gray-600 hover:border-primary-200"}`}>
                  {species}
                  {prefSpecies.includes(species) && (<CheckCircle className="w-4 h-4"/>)}
                </button>))}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="flex items-center gap-4">
              <input type="range" min="1" max="10" value={prefCapacity} onChange={(e) => setPrefCapacity(parseInt(e.target.value))} className="flex-1 accent-primary-600"/>
        ...
        })} disabled={isPending} className="flex-1 py-3 px-4 bg-primary-600 text-white rounded-xl font-bold hover:bg-primary-700 transition-all disabled:opacity-50 shadow-lg shadow-primary-200 active:scale-[0.98]">
