...
export const getFosters = catchAsync(async (req: Request, res: Response) => {
    ...
    if (req.user!.role === "shelter_staff") {
        let targetShelterId = (shelterId as string) || req.user!.shelterId?.toString();
        if (!targetShelterId) {
...
export const updateFosterStatus = catchAsync(async (req: Request, res: Response) => {
    const { id } = req.params;
    ...
    if (status === "approved" && foster.status !== "approved") {
        ...
        if (approvedShelterCount >= MAX_FOSTER_SHELTERS_PER_ADOPTER) {
            throw new AppError(400, `Adopter can be approved as foster parent for maximum ${MAX_FOSTER_SHELTERS_PER_ADOPTER} shelters`);
        }
    }
...
export const toggleFosterActive = catchAsync(async (req: Request, res: Response) => {
    const { id } = req.params;
...
export const deleteFoster = catchAsync(async (req: Request, res: Response) => {
    ...
    if (activeAssignments > 0) {
        throw new AppError(400, "Cannot delete foster parent with active assignments");
    }
...
export const getFosterAssignments = catchAsync(async (req: Request, res: Response) => {
...
const isTransactionUnsupportedError = (error: unknown): boolean => {
    ...
    return (maybeErr.code === 20 ||
        maybeErr.message?.includes("Transaction numbers are only allowed on a replica set member or mongos") === true);
};
const assignPetToFosterCore = async (req: Request, payload: {
    fosterId: string;
    petId: string;
    ...
    if (session)
        petQuery = petQuery.session(session);
    ...
    if (shelterId !== pet.shelterId.toString()) {
        ...
        if (session)
            approvedAppQuery = approvedAppQuery.session(session);
    ...
    if (session)
        fosterQuery = fosterQuery.session(session);
    ...
    if (foster.preferredSpecies && foster.preferredSpecies.length > 0) {
        ...
        if (!isMatch) {
            throw new AppError(400, `This foster parent only accepts: ${foster.preferredSpecies.join(", ")}`);
        }
    }
    let fosterUserQuery = User.findById(foster.userId);
    if (session)
        fosterUserQuery = fosterUserQuery.session(session);
    ...
    if (session)
        fosterIdsQuery = fosterIdsQuery.session(session);
    ...
    }, { $inc: { activePlacementCount: 1 } }, session ? { new: true, session } : { new: true });
    if (!updatedFosterUser) {
        throw new AppError(400, "Foster parent has reached federation-wide hard limit (3 active placements)");
    }
...
export const assignPetToFoster = catchAsync(async (req: Request, res: Response) => {
...
const takeBackPetFromFosterCore = async (req: Request, assignmentId: string, returnStatus = "available", returnNotes?: string, session?: mongoose.ClientSession) => {
    let assignmentQuery = FosterAssignment.findById(assignmentId);
    if (session)
        assignmentQuery = assignmentQuery.session(session);
    ...
    if (foster) {
        ...
        await User.findByIdAndUpdate(foster.userId, [
            {
                $set: {
    ...
    if (pet) {
        pet.status = returnStatus as "available" | "intake" | "medical_hold" | "meet" | "adopted" | "returned" | "fostered" | "transferred" | "deceased";
        if (session) {
            await pet.save({ session });
        }
        else {
...
export const takeBackPetFromFoster = catchAsync(async (req: Request, res: Response) => {
    const { id } = req.params;
    ...
    try {
        try {
            await session.withTransaction(async () => {
                updatedAssignment = await takeBackPetFromFosterCore(req, id, returnStatus, returnNotes, session);
            });
        }
        catch (error) {
            if (!isTransactionUnsupportedError(error)) {
            ...
            updatedAssignment = await takeBackPetFromFosterCore(req, id, returnStatus, returnNotes);
        }
    }
    finally {
        await session.endSession();
...
export const applyToBeFoster = catchAsync(async (req: Request, res: Response) => {
    const userId = req.user!.id;
    const { shelterId, capacity, experience, homeType, hasYard, preferredSpecies, availability, } = req.body;
    const existingApplication = await Foster.findOne({ userId, shelterId });
    if (existingApplication) {
        throw new AppError(400, "You have already applied to foster for this shelter");
    }
    ...
    if (linkedShelterCount >= MAX_FOSTER_SHELTERS_PER_ADOPTER) {
        throw new AppError(400, `You can apply as foster parent for maximum ${MAX_FOSTER_SHELTERS_PER_ADOPTER} shelters`);
    }
...
export const getAdopterFosterStatus = catchAsync(async (req: Request, res: Response) => {
    const userId = req.user!.id;
...
export const getAdopterFosterShelters = catchAsync(async (req: Request, res: Response) => {
    const userId = req.user!.id;
    const fosterRecords = await Foster.find({
        ...
        .populate("shelterId", "name")
        .sort({ createdAt: -1 });
    const shelterCount = fosterRecords.filter((record) => ["pending", "approved"].includes(record.status)).length;
    res.json({
        success: true,
        data: {
...
export const updateFosterProfile = catchAsync(async (req: Request, res: Response) => {
    const userId = req.user!.id;
    ...
    if (preferredSpecies)
        foster.preferredSpecies = preferredSpecies;
    if (capacity)
        foster.capacity = capacity;
    if (availability)
        foster.availability = availability;
    await foster.save();
    res.json({
        success: true,
...
export const updateAdopterFosterPreferencesById = catchAsync(async (req: Request, res: Response) => {
    const userId = req.user!.id;
    const { id } = req.params;
    const { preferredSpecies, capacity, availability } = req.body;
    const foster = await Foster.findOne({
    ...
    if (capacity !== undefined) {
        ...
        if (nextCapacity < foster.currentAnimals) {
            throw new AppError(400, "Capacity cannot be lower than current assigned animals");
        }
...
export const getAdopterAssignments = catchAsync(async (req: Request, res: Response) => {
    const userId = req.user!.id;
