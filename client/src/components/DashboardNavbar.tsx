...
export default function DashboardNavbar({ onMenuClick }: DashboardNavbarProps) {
    ...
    const shelterOptionsMap = new Map<string, {
        id: string;
        name: string;
        role: string;
    }>();
    memberships.forEach((m) => {
        ...
        if (!id)
            return;
        const name = typeof m.shelterId === "object" ? m.shelterId.name : "Shelter";
        if (!isValidShelterName(name))
            return;
    ...
    if (user?.shelterId) {
        ...
        if (primaryId && !shelterOptionsMap.has(primaryId)) {
            const primaryName = typeof user.shelterId === "object"
                ? user.shelterId.name
                : "Primary Shelter";
            if (!isValidShelterName(primaryName))
                return;
    ...
    (user?.staffApplications || [])
        ...
        if (!sid || shelterOptionsMap.has(sid))
            return;
        const name = typeof app.shelterId === "object"
            ? app.shelterId.name
            : "Approved Shelter";
        if (!isValidShelterName(name))
            return;
    ...
    const currentShelter = shelterOptions.find((s) => s.id === activeShelterId) || shelterOptions[0];
    useEffect(() => {
        if (effectiveRole === "shelter_staff" &&
            !activeShelterId &&
            shelterOptions.length > 0) {
            dispatch(setActiveShelter({
    ...
    return (<nav className="bg-white shadow-sm border-b border-gray-200 px-4 py-3">
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-4">
          ...
          {effectiveRole === "shelter_staff" && shelterOptions.length > 0 && (<div className="relative">
              <button onClick={() => setIsShelterSwitcherOpen(!isShelterSwitcherOpen)} className="flex items-center gap-2 px-3 py-1.5 bg-gray-50 hover:bg-gray-100 border border-gray-200 rounded-lg transition-all">
                ...
                <ChevronDown className={`w-4 h-4 text-gray-400 transition-transform ${isShelterSwitcherOpen ? "rotate-180" : ""}`}/>
              </button>

              {isShelterSwitcherOpen && (<>
                  <div className="fixed inset-0 z-40" onClick={() => setIsShelterSwitcherOpen(false)}/>
                  <div className="absolute left-0 mt-2 w-64 bg-white rounded-xl shadow-xl border border-gray-100 py-2 z-50">
                    ...
                    return (<button key={s.id} onClick={() => handleSwitchShelter(s.id, s.role)} className="w-full flex items-center justify-between px-4 py-2.5 hover:bg-primary-50 transition-colors group">
                          <div className="flex items-center gap-3">
                            <div className={`w-2 h-2 rounded-full ${isSelected ? "bg-primary-600" : "bg-transparent"}`}/>
          ...
          {!activeShelterId && (<h1 className="text-xl font-semibold text-gray-800 hidden md:block">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="relative">
            <button onClick={() => setIsProfileOpen(!isProfileOpen)} className="flex items-center gap-2 p-2 hover:bg-gray-50 rounded-lg transition-colors">
