...
export default function AssignPetModal({
  isOpen,
  onClose,
  onSubmit,
  isPending,
  ...
  const selectedFoster = fosters.find(
        ...
        >
          ...
          <div>
            ...
            >
              ...
              {availablePets
                ?.filter((pet: Pet) => {
                  if (!selectedFoster) return true;
                  if (
                    !selectedFoster.preferredSpecies ||
                    selectedFoster.preferredSpecies.length === 0
                  )
                    return true;
                  return selectedFoster.preferredSpecies.some(
                    (pref: string) => {
            ...
            {selectedFoster &&
              (availablePets || []).filter((pet: Pet) =>
                selectedFoster.preferredSpecies?.some((pref: string) => {
                  const p = pref.toLowerCase();
                  const s = pet.species.toLowerCase();
                  return p.includes(s) || s.includes(p);
                }),
