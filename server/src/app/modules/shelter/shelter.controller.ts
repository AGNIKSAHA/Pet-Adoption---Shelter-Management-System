...
export const createShelter = catchAsync(async (req: Request, res: Response) => {
  const shelter = await shelterService.createShelterInDB(
    req.body,
    req.user!.id,
    req.ip,
    req.get("user-agent"),
...
export const applyToShelter = catchAsync(
  async (req: Request, res: Response) => {
    const user = await shelterService.applyToShelterInDB(
      req.user!.id,
      req.body.shelterId,
      req.ip,
      req.get("user-agent"),
...
export const approveStaff = catchAsync(async (req: Request, res: Response) => {
  const application = await shelterService.approveStaffInDB(
    req.user!.id,
    req.params.id,
    req.ip,
    req.get("user-agent"),
...
export const rejectStaff = catchAsync(async (req: Request, res: Response) => {
  const application = await shelterService.rejectStaffInDB(
    req.user!.id,
    req.params.id,
    req.ip,
    req.get("user-agent"),
